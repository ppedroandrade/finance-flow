  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <PageHeader title="Seus cartões, sem surpresa" description="Acompanhe limites e concentre os gastos de cada cartão." action={trigger} />
      ...
      {cards.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{cards.map((card) => <CreditCardCard key={card.id} card={card} />)}</div> : <EmptyState icon={CreditCard} title="Nenhum cartão cadastrado" description="Adicione um cartão para acompanhar limite e fatura." action={<Button onClick={() => setOpen(true)}><Plus className="size-4" />Adicionar cartão</Button>} />}
